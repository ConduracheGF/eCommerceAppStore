...
public class LoginForm : Form
{
	...
	private void AttemptLogin()
	{
		...
		if (_cboRole.SelectedIndex == 1) // Admin
		{
			if (_txtPassword.Text != "admin")
			{
				MessageBox.Show("Parolă incorectă pentru Administrator! (Implicit: admin)", "Eroare", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
			SelectedRole = UserRole.Admin;
			ApiService.SetJwtToken("ADMIN_MOCK_TOKEN"); // Setează Header-ul de autentificare[cite: 35]
		}
		else // Client
		{
